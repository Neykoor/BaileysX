...
import { CarouselCardType, ListType, WAMessageStatus, WAProto } from '../Types'
...
export const prepareWAMessageMedia = async (
	...
	if (isNewsletter) {
		...
		try {
			const rawUploadData = await getRawMediaUploadData(
				uploadData.media,
				options.mediaTypeOverride || mediaType,
				logger
			)
			filePath = rawUploadData.filePath
			const { fileSha256, fileLength } = rawUploadData
...
export const generateWAMessageContent = async (
	...
	if (hasNonNullishProperty(message, 'raw')) {
		...
		const nativeButtons = (message as any).buttons.map((button: any) => {
			...
			const buttonText =
				button.text || (typeof button.buttonText === 'string' ? button.buttonText : button.buttonText?.displayText)
		...
		m.interactiveMessage = interactiveMessage
	} else if (hasNonNullishProperty(message, 'legacyButtons')) {
		const buttonsMessage: any = {
			contentText: (message as any).text || (message as any).caption || '',
			footerText: (message as any).footer || '',
			buttons: (message as any).legacyButtons.map((button: any) => ({
				buttonId: button.id || button.buttonId || button.text,
				buttonText: { displayText: button.text || button.buttonText || 'Click' },
				type: 1
			}))
		}
		...
		m.interactiveResponseMessage = {
			body: {
				format:
					message.flowReply.format ?? proto.Message.InteractiveResponseMessage.Body.Format.DEFAULT,
		...
		const pollResultSnapshotMessage: proto.Message.IPollResultSnapshotMessage = {
			...
			pollVotes: message.pollResult.votes.map(vote => ({
				...
				optionVoteCount:
					typeof vote.voteCount === 'string' ? parseInt(vote.voteCount, 10) : vote.voteCount
			}))
		}

		if (message.pollResult.quiz) {
...
export const generateWAMessageFromContent = (
	...
	if (quoted && !isJidNewsletter(jid) && key !== 'protocolMessage') {
...
export function getAggregateResponsesInEventMessage(
	...
	const responseTypes = ['GOING', 'NOT_GOING', 'MAYBE'] as const
	...
	const EventResponseType = proto.Message.EventResponseMessage.EventResponseType

	for (const update of eventResponses || []) {
		const responseEnum = update.eventResponseMessage?.response
		const responseType =
			responseEnum === EventResponseType.GOING
				? 'GOING'
				: responseEnum === EventResponseType.NOT_GOING
					? 'NOT_GOING'
					: responseEnum === EventResponseType.MAYBE
						? 'MAYBE'
						: 'UNKNOWN'
