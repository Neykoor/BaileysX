			...
			res => {
				...
				res.on('end', () => {
					try {
						...
						// El cuerpo no es JSON: normalmente porque el servidor de media de
						// WhatsApp respondió un error (4xx/5xx) con texto plano o vacío en
						// vez del JSON esperado. Antes esto se tragaba en silencio
						// (resolve(undefined)) y lo único que quedaba en los logs era el
						// "Media upload failed on all hosts" genérico al agotar los hosts,
						// sin ninguna pista de la causa real. Ahora se rechaza con el
						// status/cuerpo reales para que aparezcan en el warn de cada host.
						reject(new Error(`upload respondió HTTP ${res.statusCode ?? '?'}: ${body.slice(0, 300) || '(cuerpo vacío)'}`))
					}
				})
			}
		)
...
export const getWAUploadToServer = (
	...
	return async (filePath, { mediaType, fileEncSha256B64, timeoutMs, newsletter }) => {
		...
		if (!urls) {
			// Antes este Boom no llevaba nada del error real, así que cuando fallaban
			// TODOS los hosts (el caso reportado) lo único visible era este mensaje
			// genérico. Ahora arrastra la última causa real (HTTP/status/red) en `data`
			// para poder distinguir "problema de red hacia los hosts de media" de
			// "WhatsApp rechazó el payload" sin tener que reproducir el bug a ciegas.
			throw new Boom('Media upload failed on all hosts', {
				statusCode: 500,
				data: { lastError: lastError instanceof Error ? lastError.message : lastError }
			})
		}
