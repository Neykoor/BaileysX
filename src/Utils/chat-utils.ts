...
export const chatModificationToAppPatch = (mod: ChatModification, jid: string) => {
	...
	let patch: WAPatchCreate | WAPatchCreate[]
	if ('mute' in mod) {
		patch = {
			syncAction: {
				muteAction: {
		...
		const buildStarPatch = (key: { id: string; fromMe?: boolean }): WAPatchCreate => ({
			syncAction: {
				starAction: {
		...
		patch =
			mod.star.messages.length > 1 ? mod.star.messages.map(buildStarPatch) : buildStarPatch(mod.star.messages[0]!)
	...
	if (Array.isArray(patch)) {
		for (const p of patch) {
			p.syncAction.timestamp = stamp
		}
	} else {
		patch.syncAction.timestamp = stamp
	}
