...
export const makeMessagesSocket = (config: SocketConfig) => {
	...
	const getUSyncDevices = async (
		...
		if (result) {
			...
			await devicesMutex.mutex(async () => {
				const allFetchedUsers = new Set<string>()
				for (const jid of toFetch) {
					const fetchedUser = jidDecode(jid)?.user
					if (fetchedUser) allFetchedUsers.add(fetchedUser)
				}

				for (const fetchedUser of allFetchedUsers) {
					if (!deviceMap[fetchedUser]) {
						deviceMap[fetchedUser] = []
					}
				}
	...
	const assertSessions = async (jids: string[], force?: boolean) => {
		...
		if (jidsRequiringFetch.length) {
			const pnJidsToFetch = jidsRequiringFetch.filter(jid => !!isPnUser(jid) || !!isHostedPnUser(jid))
			const lidResults = (await signalRepository.lidMapping.getLIDsForPNs(pnJidsToFetch)) || []
			const resolvedPnUsers = new Set(lidResults.map(a => jidDecode(a.pn)?.user).filter(Boolean))
			const unresolvedPnJids = pnJidsToFetch.filter(jid => !resolvedPnUsers.has(jidDecode(jid)?.user))

			const wireJids = [
				...jidsRequiringFetch.filter(jid => !!isLidUser(jid) || !!isHostedLidUser(jid)),
				...lidResults.map(a => a.lid),
				...unresolvedPnJids
			]

			logger.debug({ jidsRequiringFetch, wireJids }, 'fetching sessions')
			const KEY_FETCH_CHUNK_SIZE = 10
			const KEY_FETCH_CHUNK_DELAY_MS = 250
			for (let i = 0; i < wireJids.length; i += KEY_FETCH_CHUNK_SIZE) {
				if (i > 0) {
					await delay(KEY_FETCH_CHUNK_DELAY_MS)
				}

				const wireJidsChunk = wireJids.slice(i, i + KEY_FETCH_CHUNK_SIZE)
				const result = await query({
					tag: 'iq',
					attrs: {
						xmlns: 'encrypt',
	...
	const SEND_GROUP_METADATA_MAX_AGE_MS = 30_000
	...
	const relayMessage = async (
		...
		let pendingSenderKeyMemory: { jid: string; map: { [_: string]: boolean } } | undefined
		...
		await authState.keys.transaction(async () => {
			...
			if (isGroupOrStatus && !isRetryResend) {
				const [groupData, senderKeyMap] = await Promise.all([
					(async () => {
						...
						if (groupData && Array.isArray(groupData?.participants)) {
							...
							groupData = await groupMetadata(jid, SEND_GROUP_METADATA_MAX_AGE_MS)
						}
				...
				pendingSenderKeyMemory = { jid, map: senderKeyMap }
			} else {
				let ownId = meId
				if (isLid && meLid) {
					ownId = meLid
			...
			if (pendingSenderKeyMemory) {
				await authState.keys.set({ 'sender-key-memory': { [pendingSenderKeyMemory.jid]: pendingSenderKeyMemory.map } })
			}
	...
	const smgss = new Smgss(waUploadToServer, relayMessage, config, sock, messageMutex, upsertMessage)
	...
	const messagesSock = {
		...
		sendMessage: async (jid: string, content: AnyMessageContent, options: MiscMessageGenerationOptions = {}) => {
			...
			if (smgssType) {
				...
				const built = await smgss.handle(smgssType, content, jid, quoted, options)

				if (smgssType === 'ALBUM' || smgssType === 'EVENT' || smgssType === 'POLL_RESULT' || smgssType === 'GROUP_STORY') {
					return built as WAMessage
				}
