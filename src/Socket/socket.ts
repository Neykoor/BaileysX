...
export const makeSocket = (config: SocketConfig) => {
	...
	const MAX_RETIRED_SIGNED_PRE_KEYS = 3
	...
	const rotateSignedPreKey = async (): Promise<void> => {
		...
		const retiredIds = [...(creds.retiredSignedPreKeyIds || []), oldSignedPreKey.keyId]
		const idsToDrop = retiredIds.slice(0, Math.max(0, retiredIds.length - MAX_RETIRED_SIGNED_PRE_KEYS))
		const keptIds = retiredIds.slice(idsToDrop.length)
