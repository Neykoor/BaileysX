...
export const makeSocket = (config: SocketConfig) => {
	...
	const query = async (node: BinaryNode, timeoutMs?: number) => {
		...
		const waitPromise = waitForMessage<any>(msgId, timeoutMs)
	...
	const SIGNED_PRE_KEY_GRACE_PERIOD_MS = 3 * 24 * 60 * 60 * 1000

	const rotateSignedPreKey = async (): Promise<void> => {
		const oldSignedPreKey = creds.signedPreKey
		const oldPrivateKey = oldSignedPreKey.keyPair.private
		const newId = (oldSignedPreKey.keyId || 0) + 1
		...
		const retiredKey: RetiredSignedKeyPair = {
			keyId: oldSignedPreKey.keyId,
			signature: oldSignedPreKey.signature,
			timestampS: oldSignedPreKey.timestampS,
			keyPair: {
				public: oldSignedPreKey.keyPair.public,
				private: Buffer.from(oldPrivateKey)
			},
			retiredAtMs: Date.now()
		}

		const trackedIds = creds.retiredSignedPreKeyIds || []
		const existingRetired = trackedIds.length
			? await keys.get('retired-signed-pre-key', trackedIds.map(String))
			: {}
		...
		for (const id of trackedIds) {
			const entry = existingRetired[String(id)]
			if (entry && now - entry.retiredAtMs < SIGNED_PRE_KEY_GRACE_PERIOD_MS) {
				keptIds.push(id)
			} else {
				idsToDrop.push(id)
			}
		}
		keptIds.push(oldSignedPreKey.keyId)

		const keyUpdate: { [id: string]: RetiredSignedKeyPair | null } = {
			[String(oldSignedPreKey.keyId)]: retiredKey
		}
		for (const id of idsToDrop) {
			keyUpdate[String(id)] = null
		}

		await keys.set({ 'retired-signed-pre-key': keyUpdate })

		ev.emit('creds.update', { signedPreKey: skey, retiredSignedPreKeyIds: keptIds })
	...
	const looksLikeUsername = (value: string) => !value.includes('@') && /[^\d+\s]/.test(value)

	const onWhatsApp = async (...userInputs: string[]) => {
		let usyncQuery = new USyncQuery()

		let contactEnabled = false
		const ensureContactProtocol = () => {
			if (!contactEnabled) {
				contactEnabled = true
		...
		for (const input of userInputs) {
			...
			if (isLidUser(jid)) {
				const resolvedPn =
					signalRepository.lidMapping.phoneCache.getPhoneForLid(jid) ||
					(await signalRepository.lidMapping.getPNForLID(jid))

				if (!resolvedPn) {
					logger?.warn(`no cached PN mapping for LID ${jid}, skipping onWhatsApp lookup`)
					continue
				}

				ensureContactProtocol()
				const phone = `+${resolvedPn.replace('+', '').split('@')[0]?.split(':')[0]}`
			...
			if (looksLikeUsername(stripped)) {
				ensureContactProtocol()
				usyncQuery.withUser(new USyncUser().withUsername(stripped))
				continue
			}

			ensureContactProtocol()
			const phone = `+${stripped.replace('+', '').split('@')[0]?.split(':')[0]}`
			usyncQuery.withUser(new USyncUser().withPhone(phone))
		}
	...
	ws.on('CB:iq,type:set,pair-device', async (stanza: BinaryNode) => {
		...
		const genPairQR = () => {
			...
			const qr = buildPairingQRData(ref, noiseKeyB64, identityKeyB64, advB64)
