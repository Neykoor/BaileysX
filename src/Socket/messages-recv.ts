...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const sendRetryRequest = async (node: BinaryNode, forceIncludeKeys = false) => {
		...
		if (retryCount <= 2) {
			...
			if (messageRetryManager) {
				...
				messageRetryManager.schedulePhoneRequest(msgId, msgKey?.participant || undefined, async () => {
					try {
	...
	const handleGroupNotification = (fullNode: BinaryNode, child: BinaryNode, msg: Partial<WAMessage>) => {
		...
		switch (child?.tag) {
			case 'create':
				const metadata = extractGroupMetadata(child, signalRepository.lidMapping)
			...
			await messageMutex.mutex(async () => {
				...
				if (msg.messageStubType === proto.WebMessageInfo.StubType.CIPHERTEXT && msg.category !== 'peer') {
					...
					if (messageRetryManager && msg.key.id) {
						messageRetryManager.cancelPendingPhoneRequest(msg.key.id, msg.key.participant || undefined)
					}
