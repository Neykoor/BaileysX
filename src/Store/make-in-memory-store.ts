...
import { md5, toNumber, updateMessageWithEventResponse, updateMessageWithPollUpdate, updateMessageWithReaction, updateMessageWithReceipt } from '../Utils/index.js'
	...
	const bind = (ev: BaileysEventEmitter) => {
		...
		ev.on('messages.update', updates => {
			for (const { update, key } of updates) {
				...
				const existing = key.id ? list.get(key.id) : undefined
				let remainingUpdate = update
				if (existing && (update.eventResponses?.length || update.pollUpdates?.length)) {
					const { eventResponses, pollUpdates, ...rest } = update

					for (const response of eventResponses || []) {
						updateMessageWithEventResponse(existing, response)
					}

					for (const vote of pollUpdates || []) {
						updateMessageWithPollUpdate(existing, vote)
					}

					remainingUpdate = rest
				}

				const result = key.id ? list.updateAssign(key.id, remainingUpdate) : false
				if (!result) {
